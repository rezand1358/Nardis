...
class SoapCategory extends SoapCall{
  SoapCall soapCall;
  BuildContext context; 
  SoapOpers soapOpers;

  SoapCategory({this.context})
  {
    soapOpers=new SoapOpers(context: context,type:10,oper:SoapOpersConstants.OPERS_CATEGORY);
  }
...
''';
  ...
  action=SoapConstants.SOAP_ACTION_CATEGORY;
  responseTag= 'GetProduct_GroupResponse';
  resultTag= 'GetProduct_GroupResult';
  List<dynamic> result=await applySoap(method, SoapTypes.CATEGORY);
//   soapCall=new SoapCall(action:SoapConstants.SOAP_ACTION_CATEGORY,requestBody: envelope,responseTag: 'GetProduct_GroupResponse',resultTag: 'GetProduct_GroupResult');
//   List<dynamic> result=await soapCall.runSoap(SoapConstants.METHOD_NAME_CATEGORY,this);
//   if(result!=null &&
//   result.length>0){
...
//    server.simulateMessage("CATEGORY_LOADED");
  ...
  void doActions(List result) {
    if(result!=null &&
    result.length>0) {
    loadCategory(jsonDecode(result[0])); 
    }
    else{
    RxBus.post(ChangeEvent(message: 'CATEGORY_LOADED'));
    }
